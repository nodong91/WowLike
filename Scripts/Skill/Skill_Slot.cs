...
public class Skill_Set : MonoBehaviour
{
    ...
    void TargetHIt(Unit_AI _target, Transform _lastProjcetile)
    {
        ...
        _target.TakeDamage(fromUnit, _lastProjcetile.position, damage, skillStruct);// 타겟
    }

    void SplashArea(Transform _lastProjcetile)
    {
        foreach (var child in Game_Manager.current.GetUnitDict)
        {
            ...
            if (VisibleTarget(_lastProjcetile, target) == true)
            {
                ...
                float damage = fromUnit.GetDamage;
                target.TakeDamage(fromUnit, _lastProjcetile.position, damage, skillStruct);// 광역
            }
        }
    }
